...
public abstract class ShiroAccessControlFilter extends PathMatchingFilter {
	...
	private static final Logger logger = LoggerFactory.getLogger(ShiroAccessControlFilter.class);
	...
	/**
	 * <p>
	 * 提供了访问控制的基础功能；比如是否允许访问/当访问拒绝时如何处理等<br>
	 * <code>isAccessAllowed</code>：表示是否允许访问；<br>
	 * <code>mappedValue</code>：就是[urls]配置中拦截器参数部分，如果允许访问返回true，否则false；<br>
	 * </p>
	...
	/**
	 * 表示当访问拒绝时是否已经处理了；如果返回true表示需要继续处理；如果返回false表示该拦截器实例已经处理了，将直接返回即可。<br>
	...
	protected boolean isLoginRequest(ServletRequest request,ServletResponse response) {
		...
		if (loginUrlMap != null) {
			for (String key : loginUrlMap.keySet()) {
				/*
				 * 方法pathsMatch：用于path与请求路径进行匹配的方法；如果匹配返回true；
				 */

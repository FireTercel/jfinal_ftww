...
public class OperatorService extends BaseService {
	...
	public boolean delete(String ids){
		...
		//删除数据库数据
		return Operator.dao.deleteById(ids);
	...
	public String childNodeData(String moduleIds){
		...
		if(null == moduleIds){
			//  1.模块功能初始化调用（获取模块根节点）
			String sql = getSql("basic.operator.rootModule");
			ModuleList = Module.dao.find(sql);
			
		} else if (null != moduleIds) {
			moduleIds = moduleIds.replace("module_", "");
			
			// 2.通用子节点查询
			String sqlModule = getSql("basic.operator.childModule");
			ModuleList = Module.dao.find(sqlModule, moduleIds);	
			
			String sqlOperator = getSql("basic.operator.byModuleIds");
		...
		for (Module module : ModuleList) {
			...
			sb.append(" }");
			if (ModuleList.indexOf(module) < moduleIndexSize) {
				sb.append(", ");
			}
		}
	...
	public void list(SplitPage splitPage){
		...
		splitPageBase(DictKeys.db_dataSource_main,splitPage ,select, "basic.operator.splitPage");
	}

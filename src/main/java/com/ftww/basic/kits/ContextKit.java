...
public class ContextKit {
	...
	public static void outPage(HttpServletResponse response, String content) {
		...
		response.setCharacterEncoding(StringKit.encoding);
		// PrintWriter out = response.getWriter();
		// out.print(content);
		try {
			response.getOutputStream().write(content.getBytes(StringKit.encoding));// char to byte 性能提升
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void outDownCsv(HttpServletResponse response,String content){
		...
		try {
			response.setHeader("Content-Disposition", "attachment; filename=" + java.net.URLEncoder.encode(DateTimeKit.format(DateTimeKit.getDate(), DateTimeKit.pattern_ymd_hms_s) + ".csv", StringKit.encoding));
		...
		// out.write(content);
		try {
			response.getOutputStream().write(content.getBytes(StringKit.encoding));
		} catch (IOException e) {
			e.printStackTrace();
		}// char to byte 性能提升
			// out.flush();
	...
	public static User getCurrentUser(HttpServletRequest request,boolean userAgentVali){
		...
		if (null != loginCookie && !loginCookie.equals("")) {
			...
			try {
				//解密后数据
				dataByte = SecurityIDEAKit.decrypt(securityByte, keyByte);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static void setCurrentUser(HttpServletRequest request, HttpServletResponse response, User user, boolean autoLogin) {
		...
		long date =  DateTimeKit.getDateByTime();
		...
		try {
			securityByte = SecurityIDEAKit.encrypt(authTokenByte, keyByte);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		// 5. 认证cookie Base64编码
		try {
			securityCookie = StringKit.encode(securityCookie);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void setAuthCode(HttpServletResponse response, String authCode){
		byte[] authTokenByte = null;
		try {
			authTokenByte = authCode.getBytes(StringKit.encoding);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		String securityKey = (String) PropertiesPlugin.getParamMapValue(DictKeys.config_securityKey_key);
		...
		WebKit.addCookie(response,  "", "/", true, "authCode", securityCookie, maxAgeTemp);
	}
	...
	public static String getAuthCode(HttpServletRequest request){
		String authCode = WebKit.getCookieValueByName(request, "authCode");
		if (null != authCode && !authCode.equals("")) {
			// Base64解码
			try {
				authCode = StringKit.decode(authCode);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static String getParam(HttpServletRequest request, String name) {
		...
		if (null != value && !value.isEmpty()) {
			try {
				value = URLDecoder.decode(value, StringKit.encoding).trim();
			} catch (UnsupportedEncodingException e) {
	...
	public static String requestStream(HttpServletRequest request) {
		...
		try {
			request.setCharacterEncoding(StringKit.encoding);
			inputStream = (ServletInputStream) request.getInputStream();
			inputStreamReader = new InputStreamReader(inputStream, StringKit.encoding);

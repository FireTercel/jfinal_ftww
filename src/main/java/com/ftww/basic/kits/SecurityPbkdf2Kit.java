 ...
 * 再次强调，不要存储明文密码，不要存储明文密码，因为没必要！不要担心将盐和密文存储在同一张表中，上面已经说过了，这个无关紧要。<br>
 * 
 * 2.当认证用户的时候，从数据库中取出盐和密文，将他们和明文密码同时传给authenticate()，根据返回结果判断是否认证成功。<br>
 * 
 * 3.当用户修改密码的时候，仍然可以使用原来的盐，只需要调用getEncryptedPassword()方法重新生成密文就可以了。<br>
 */
public class SecurityPbkdf2Kit {

	private static Logger log = Logger.getLogger(SecurityPbkdf2Kit.class);

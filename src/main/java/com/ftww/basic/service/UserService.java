...
import com.ftww.basic.kits.SecurityPbkdf2Kit;
...
public class UserService extends BaseService {
	...
	public void save(User user, String password, UserInfo userInfo) {
		try {
			// 密码加密
			byte[] salt = SecurityPbkdf2Kit.generateSalt();// 密码盐
			byte[] encryptedPassword = SecurityPbkdf2Kit.getEncryptedPassword(password, salt);
			user.set("salt", salt);
	...
	public void list(SplitPage splitPage) {
		...
		splitPageBase(DictKeys.db_dataSource_main, splitPage, select, "basic.user.splitPage");
	}
	...
	public boolean valiPassWord(String userName, String passWord) {
		try {
			...
			boolean bool = SecurityPbkdf2Kit.authenticate(passWord, encryptedPassword, salt);
			if (bool) {
				return true;
			}
		} catch (Exception e) {
	...
	public void passChange(String userName, String passOld, String passNew){
		try {
			Map<String, Object> param = new HashMap<String, Object>();
			param.put("column", "userName"); 
			String sql = getSql("basic.user.column", param);
			User user = User.dao.findFirst(sql, userName);
			...
			boolean bool = false;
			try {
				bool = SecurityPbkdf2Kit.authenticate(passOld, encryptedPassword, salt);
			} catch (NoSuchAlgorithmException e) {
				e.printStackTrace();
			} catch (InvalidKeySpecException e) {
				e.printStackTrace();
			...
			if (bool) {
				byte[] saltNew = SecurityPbkdf2Kit.generateSalt();// 密码盐
				byte[] encryptedPasswordNew = SecurityPbkdf2Kit.getEncryptedPassword(passNew, saltNew);

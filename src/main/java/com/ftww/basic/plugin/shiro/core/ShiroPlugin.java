...
import com.ftww.basic.plugin.shiro.core.handler.AuthenticatedAuthzHandler;
import com.ftww.basic.plugin.shiro.core.handler.AuthzHandler;
import com.ftww.basic.plugin.shiro.core.handler.CompositeAuthzHandler;
import com.ftww.basic.plugin.shiro.core.handler.GuestAuthzHandler;
import com.ftww.basic.plugin.shiro.core.handler.PermissionAuthzHandler;
import com.ftww.basic.plugin.shiro.core.handler.RoleAuthzHandler;
import com.ftww.basic.plugin.shiro.core.handler.UserAuthzHandler;
...
public class ShiroPlugin implements IPlugin{
	...
	/**
	 * 匹配到多个url时，false：表示匹配到多个，就必须多个权限。true：表示只需要第一个匹配到的权限匹配。
	 * 如/a/b，可以匹配到/a/** 和/a/b** 两个权限 
	 * 匹配到一个就算通过或者两个权限都需要才通过？
	 */
	...
	public boolean start() {
		...
		for(Entry<String,Class<? extends Controller>> entry : routes.getEntrySet()){
			...
			List<Annotation> controllerAnnotations = getAuthzAnnotation(controllerClass);
	...
	/**
	 * 从Controller方法中构建出需要排除的方法列表？<br>
	 * 避免与自定义的Controller中的Action混淆
	...
	/**
	 * 逐个扫描注解，若是相应的注解则在相应的位置赋值。 注解的处理是有顺序的，
	 * 依次为RequiresRoles，
	 * RequiresPermissions，
	 ...
	 * @return
	 */
	private List<Annotation> getAuthzAnnotation(Class<? extends Controller> targetClass){
		List<Annotation> annotations = new ArrayList<Annotation>();
		for(Class<? extends Annotation> annClass : AUTHZ_ANNOTATION_CLASSES){
